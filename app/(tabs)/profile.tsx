...
export default function ProfileScreen() {
  ...
  const handleSync = async () => {
      ...
      try {
          ...
          Alert.alert("Sync Complete", "Your resumes are up to date with the cloud.");
      } catch (e) {
          Alert.alert("Sync Failed", "Check your internet connection or DB configuration.");
      }
  };
    ...
    <SafeAreaView className="flex-1 bg-background">
      <View className="px-6 py-4 border-b border-border bg-surface flex-row justify-between items-center">
        ...
        <TouchableOpacity onPress={() => setIsEditing(!isEditing)}>
             <Text className="text-primary font-medium">{isEditing ? 'Cancel' : 'Edit'}</Text>
        </TouchableOpacity>
      </View>
      ...
      <ScrollView contentContainerStyle={{ padding: 24 }}>
        ...
        <View className="bg-surface p-6 rounded-2xl border border-border mb-6 items-center shadow-sm">
            ...
            {isEditing ? (
                ...
                <>
                    <Text className="text-xl font-bold text-text mb-1">{user?.name || 'Guest User'}</Text>
                    <Text className="text-textSecondary text-base mb-4">{user?.email}</Text>
                </>
            )}
...
const SettingItem = ({ icon, title, value, isLast, highlight = false }: { icon: any, title: string, value: string, isLast: boolean, highlight?: boolean }) => (
  <View className={`p-4 flex-row items-center justify-between bg-surface ${!isLast ? 'border-b border-border' : ''}`}>
    <View className="flex-row items-center gap-3">
      <View className="w-8 h-8 rounded-full bg-surface-active items-center justify-center">
        <MaterialIcons name={icon} size={20} color={highlight ? COLORS.primary : COLORS.textSecondary} />
      </View>
      <Text className={`font-medium ${highlight ? 'text-primary' : 'text-text'}`}>{title}</Text>
    </View>
